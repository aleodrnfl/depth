...
def wav_to_mel(wav_path, output_size=(256, 256)):
    ...
    mel_image = mel_resized[np.newaxis, ...]    # (1, 512, 512)
    return mel_image
    ...
    def __getitem__(self, idx):
        try:
            ...
            for data in sounds:
                ...
                sound = np.concatenate([left_ir, right_ir], axis=0) # (3, 512, 512)
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.outputs = nn.Conv2d(64, 1) # 커널 사이즈 1
...
# """ 모델 중간부터 다시 돌릴 때 """
# train_checkpoint_path = "/home/mihyun/server_data/model/model2/0123_output/train_0209_30.pth"
...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch [{epoch+1}/{num_epochs}]")): 
        if batch is None:
            continue 
        rgb, depth, sound_tensor = batch # (B, 3, H, W), (B, 3, H, W), (B, 1, H, W), (B, 20, 3, H, W)
        sound_losses = []   
        
        for sound_idx in range(sound_tensor.size(1)): 
            ir = sound_tensor[:, sound_idx]  # (B, 3, H, W)
            
            inputs = torch.cat([rgb, ir], dim=1)   # (B, 9, H, W)
            inputs, targets = inputs.to(device), depth.to(device)
